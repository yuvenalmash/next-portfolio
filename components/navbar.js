...
const Navbar = () => {
  ...
  return (
    <nav className="fixed w-full z-20 backdrop-blur-3xl">
      <div className="container mx-auto px-4 py-2 md:px-6 md:py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex-shrink-0">
            <Link href="/">
              <svg className="h-9 w-9 md:h-16 md:w-16 p-1 fill-current border rounded border-current transition-colors durstion-300 ease-in-out hover:fill-cyan-500 hover:border-cyan-800 hover:animate-pulse" viewBox="0 0 8 4" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="hidden md:block">
            <ul className="flex space-x-4">
              {Object.keys(links).map((link, index) => (
                <li key={index}>
                  <Link href={links[link]} className="text-2xl group font-boxpixies transition-color duration-200 ease-out hover:text-cyan-500">
          ...
          <div className="md:hidden flex items-center">
            <button
              className="mobile-menu-button flex items-center transition-color duration-300 ease-in-out hover:text-cyan-500"
      ...
      {/* mobile menu */}
      <div
        className={`md:hidden mobile-menu fixed right-0 w-1/2 rounded backdrop-blur-sm transition-all duration-300 ${
      ...
      >
        <ul className="px-2 pt-2 pb-4 space-y-2">
          {Object.keys(links).map((link, index) => (
            <li key={index}>
              <Link href={links[link]} onClick={toggleMobileMenu} className="hover:text-orange-600 text-2xl font-boxpixies">
                {link}
              </Link>
            </li>
          ))}
